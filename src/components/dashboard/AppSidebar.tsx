...
export default function Sidebar() {
  ...
  // Override isAuthenticated in development
  const { isAuthenticated: authStatus } = useAuth()
  const isAuthenticated = isDevelopment ? true : authStatus
  ...
  const disabledClass = !isAuthenticated ? 'opacity-70 blur-[0.3px]' : ''

  return (
    <aside className="relative w-64 h-screen flex flex-col bg-white border-r border-gray-200 font-['Outfit']">
      ...
      {/* Existing sidebar content with modified disabled class */}
      <div className={disabledClass}>
        ...
        <div className={`mt-auto p-6 space-y-4 ${disabledClass}`}>
          <button disabled={!isAuthenticated} className="w-full flex items-center justify-center py-3 px-4 rounded-full bg-[#f3f5f7] text-[#1e3a8a] font-medium">
          ...
          <button disabled={!isAuthenticated} className="w-full flex items-center text-[#ff4423] font-medium">
